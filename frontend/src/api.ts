...
export const renewDemoSala=(codigo:string,hours=4):Promise<DemoSala>=>api<DemoSala>(`/api/reunion-demo/salas/${codigo}/renovar`,{method:"POST",body:JSON.stringify({rotate_token:true,hours})});
export const fetchDemoSalaAccess=(codigo:string):Promise<{estado:string;guest_url?:string|null;expires_at:string;detalle?:string}>=>api(`/api/reunion-demo/salas/${codigo}/acceso`);
