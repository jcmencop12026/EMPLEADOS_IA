...
export function PresentacionEjecutivaPage() {
  ...
  async function mostrarAlGerente(payload: Record<string, unknown>) {
    if (!sala) return;
    try { const r=await updateDemoSala(sala.codigo,{tema:temaEnVivo,visible:{...payload,nivel:nivelRevelacion,audiencia}}); setSala({...sala,...r}); } catch(e){setSalaError(e instanceof Error?e.message:"No se pudo sincronizar");}
  }
