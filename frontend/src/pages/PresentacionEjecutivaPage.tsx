      ...
      {reunionIniciada && (() => {
        ...
        return (
          <section className={`panel demo-live-panel ${vistaReunion==="TEMA"?"meeting-main-active":"meeting-main-hidden"}`}>
            ...
            <div className="meeting-copilot-dock" aria-label="ELIA copiloto siempre visible">
              <div className="meeting-copilot-dock__brand"><b>✦</b><span>ELIA · MI COPILOTO</span></div>
              <div className="meeting-copilot-dock__prompts">
                {["¿Qué digo ahora?","Profundiza este hallazgo","¿Qué datos debo pedir?","Anticipa una objeción","Sugiere siguiente paso"].map((q)=><button key={q} type="button" onClick={()=>setPreguntaElia(q)}>{q}</button>)}
              </div>
              <form className="meeting-copilot-dock__ask" onSubmit={preguntarElia}><input value={preguntaElia} onChange={(e)=>setPreguntaElia(e.target.value)} placeholder="Preguntar a ELIA…" /><button type="submit" disabled={eliaLoading||!preguntaElia.trim()}>{eliaLoading?"…":"Preguntar"}</button></form>
            </div>
