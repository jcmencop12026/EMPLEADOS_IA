...
export function PresentacionEjecutivaPage() {
  ...
  return (
    <div className={`ops-page presentacion-ejecutiva-page ${reunionIniciada ? "meeting-mode" : ""}`}>
      ...
      {reunionIniciada && <section className="manager-inbox-v23"><div><b>↙ ENTRADAS DEL GERENTE</b><span>{sala?.intereses?.length ? sala.intereses.length+" interacción(es) recibida(s)" : "Sin entradas nuevas"}</span></div>{sala?.intereses?.length?<><strong>{sala.intereses[sala.intereses.length-1].accion.replaceAll("_"," ")}</strong><p>{sala.intereses[sala.intereses.length-1].detalle||sala.intereses[sala.intereses.length-1].tema}</p><button onClick={()=>setPreguntaElia(sala.intereses?.[sala.intereses.length-1]?.detalle||"")}>Responder con ELIA →</button></>:<p>Las preguntas, solicitudes de datos y decisiones enviadas desde la sala aparecerán aquí.</p>}</section>}
