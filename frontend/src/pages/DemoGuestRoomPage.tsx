...
export function DemoGuestRoomPage(){
 ...
 const openFinding=(i:number)=>{setFocus(i);const o=EXECUTIVE_OPPORTUNITIES[i];if(o)setImpact(o.impact);setDetailOpen(false)};
 return <main className="guest-room guest-room-v8">
  ...
  <section className="guest-v17-summary">
   ...
   <div className="guest-v17-kpis">
    <button onClick={()=>{setImpact("Ingresos");setFocus(0)}}><b>↗</b><strong>+$185 M</strong><span>Nuevos ingresos</span></button>
    <button onClick={()=>{setImpact("Pérdidas");setFocus(1)}}><b>💰</b><strong>+$128 M</strong><span>Caja recuperable</span></button>
    <button onClick={()=>{setImpact("Productividad");setFocus(2)}}><b>⏱</b><strong>$96 M</strong><span>Facturación por acelerar</span></button>
    <button onClick={()=>{setImpact("Costos");setFocus(3)}}><b>◉</b><strong>$54 M</strong><span>Costos evitables</span></button>
   </div>
  </section>
  ...
  <section className="guest-workspace guest-workspace-v8">
   {panel==="hallazgo"&&<div className="guest-v17-layout">
    <article className="guest-v17-focus">
     <header><span>{EXECUTIVE_OPPORTUNITIES[focus]?.icon||"◈"} OPORTUNIDAD {String(focus+1).padStart(2,"0")}</span><b>{EXECUTIVE_OPPORTUNITIES[focus]?.priority||"OPORTUNIDAD"}</b></header>
     <strong className="guest-v17-money">{EXECUTIVE_OPPORTUNITIES[focus]?.value||IMPACT_DETAIL[impact].value}</strong>
     <h2>{EXECUTIVE_OPPORTUNITIES[focus]?.title||String(current).replace(/^Oportunidad:\s*/,"")}</h2>
     <p>{narrative}</p>
    ...
    <aside className="guest-v17-radar"><header><div><span>RADAR DE OPORTUNIDADES</span><strong>Señales que merecen atención gerencial</strong></div><small>clic para profundizar</small></header>
     <div>{EXECUTIVE_OPPORTUNITIES.map((o,i)=><button key={o.title} className={focus===i?"active":""} onClick={()=>openFinding(i)}><b>{o.icon}</b><strong>{o.value}</strong><span>{o.title}</span><em>{o.priority}</em></button>)}</div>
    </aside>
   </div>}
