...
def _build_payload(entrada: dict) -> dict:
    ...
    if cid == "V2-OP-A":
        ...
        if ctx.get("evidencia_insuficiente") or not ctx.get("datos_financieros_suficientes"):
            ...
            if ctx.get("evidencia_insuficiente"):
                payload["evidencia_insuficiente"] = True
            if ctx.get("datos_financieros_suficientes") is False:
                payload["datos_financieros_suficientes"] = False
    elif cid == "V2-OP-C":
        payload.update({
...
def run_case(db, entrada: dict, state: dict) -> dict:
    ...
    if r.get("opportunity_id"):
        ...
        if opp and opp.estado == "PENDIENTE_APROBACION" and cid == "V2-OP-A":
            ...
            result["oportunidad"] = _opp_snapshot(db, opp.id)
    elif cid == "V2-OP-B":
        result["senal_observada"] = r.get("opportunity_id") is None
    state[cid] = result
    return result
...
def main() -> int:
    ...
    try:
        ...
        ctrl = Path(__file__).resolve().parent / "00_CONTROL"
        ctrl.mkdir(parents=True, exist_ok=True)
        (ctrl / "resumen_fase_ciega_v2.json").write_text(
            json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8",
        )
        (ctrl / "FASE_CIEGA_CERRADA.md").write_text(
            f"# FASE CIEGA CERRADA — V2 R2\n\n"
            f"- **HEAD funcional:** `{head}`\n"
            f"- **Timestamp UTC:** {summary['timestamp']}\n"
            f"- **Casos ejecutados:** 12/12\n"
            f"- **Oráculo consultado:** NO\n"
            f"- **Congelado:** `{freeze.name}`\n",
            encoding="utf-8",
        )
