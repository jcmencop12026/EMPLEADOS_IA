...
DEFAULT_TRANSVERSAL = [
    {
        "capability_code": "capacidad_deterministica",
...
def _is_deterministic_record(record: FinOpsRecord) -> bool:
    """Ejecución sin LLM: costo tokens/modelo = 0 (no depende de módulos externos)."""
    ref = (record.execution_ref or "").lower()
    if "deterministic" in ref or ref.startswith("deterministic:"):
        return True
    if record.category == "Modelo IA":
        return False
    if (record.tokens_in or 0) == 0 and (record.tokens_out or 0) == 0:
        return True
    return False
