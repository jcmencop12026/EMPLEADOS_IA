...
class GobiernoNegocioApprovalAdapter:
    """Autoridad productiva: decisiones de aprobación comercial vía Gobierno Operacional."""

    def __init__(self) -> None:
        self._local = LocalNegocioApprovalAdapter()

    def required_levels(self, db: Session, org_id: str, proposal_id: str) -> list[str]:
        return self._local.required_levels(db, org_id, proposal_id)

    def ensure_records(self, db: Session, org_id: str, proposal_id: str, version_number: int) -> list[NegocioApprovalRecord]:
        return self._local.ensure_records(db, org_id, proposal_id, version_number)

    def can_present(self, db: Session, org_id: str, proposal_id: str, version_number: int) -> tuple[bool, list[str]]:
        return self._local.can_present(db, org_id, proposal_id, version_number)

    def reset_for_version(self, db: Session, org_id: str, proposal_id: str, version_number: int) -> None:
        self._local.reset_for_version(db, org_id, proposal_id, version_number)
        ...
        *,
        ...
        solicitud = gov_svc.crear_solicitud(
            ...
            user.id,
            {
                ...
                "recurso_tipo": "negocio.propuesta",
                "recurso_id": proposal_id,
                "descripcion": f"Aprobación comercial nivel {nivel}",
                "payload": {"nivel": nivel, "proposal_id": proposal_id, "version_number": ver},
                "criticidad": "HIGH",
                "motivo_solicitud": comentario,
            },
        )
        ...
        return self._local.approve(
            db, user, org_id, proposal_id, nivel, comentario=comentario, version_number=ver,
        )


def get_approval_adapter() -> ApprovalPort:
    return GobiernoNegocioApprovalAdapter()
