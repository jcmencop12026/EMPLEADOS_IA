...
def upgrade() -> None:
    ...
    with op.batch_alter_table("impl_proyectos") as batch_op:
        batch_op.create_foreign_key("fk_impl_proy_opp", "opportunities", ["opportunity_id"], ["id"])
        batch_op.create_foreign_key("fk_impl_proy_contract", "negocio_contract_records", ["contract_id"], ["id"])
        batch_op.create_foreign_key("fk_impl_proy_doc", "negocio_proposal_documents", ["documento_contrato_id"], ["id"])
        batch_op.create_foreign_key("fk_impl_proy_budget", "finops_budgets", ["finops_budget_id"], ["id"])
    ...
    with op.batch_alter_table("exito_renovaciones") as batch_op:
        batch_op.create_foreign_key("fk_renov_opp", "opportunities", ["opportunity_id"], ["id"])
    with op.batch_alter_table("exito_expansiones") as batch_op:
        batch_op.create_foreign_key("fk_exp_opp", "opportunities", ["opportunity_id"], ["id"])

    op.add_column("negocio_contract_records", sa.Column("finops_budget_id", sa.String(36), nullable=True))
    with op.batch_alter_table("negocio_contract_records") as batch_op:
...
def downgrade() -> None:
    with op.batch_alter_table("negocio_contract_records") as batch_op:
        batch_op.drop_constraint("fk_contract_finops_budget", type_="foreignkey")
    op.drop_column("negocio_contract_records", "finops_budget_id")
    with op.batch_alter_table("exito_expansiones") as batch_op:
        batch_op.drop_constraint("fk_exp_opp", type_="foreignkey")
    with op.batch_alter_table("exito_renovaciones") as batch_op:
        batch_op.drop_constraint("fk_renov_opp", type_="foreignkey")
    ...
    with op.batch_alter_table("impl_proyectos") as batch_op:
        batch_op.drop_constraint("fk_impl_proy_budget", type_="foreignkey")
        batch_op.drop_constraint("fk_impl_proy_doc", type_="foreignkey")
        batch_op.drop_constraint("fk_impl_proy_contract", type_="foreignkey")
        batch_op.drop_constraint("fk_impl_proy_opp", type_="foreignkey")
