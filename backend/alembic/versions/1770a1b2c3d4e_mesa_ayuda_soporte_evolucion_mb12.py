...
revision = "1770a1b2c3d4e"
down_revision = "1760a1b2c3d4e"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("support_cases") as batch_op:
        batch_op.create_foreign_key("fk_support_case_coordinador", "users", ["coordinador_id"], ["id"])
        batch_op.create_foreign_key("fk_support_case_resp_tecnico", "users", ["responsable_tecnico_id"], ["id"])
        batch_op.create_foreign_key("fk_support_case_resp_funcional", "users", ["responsable_funcional_id"], ["id"])
    ...
    with op.batch_alter_table("support_cases") as batch_op:
        batch_op.create_foreign_key("fk_support_case_problema", "support_problems", ["problema_id"], ["id"])
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("support_cases") as batch_op:
        batch_op.drop_constraint("fk_support_case_problema", type_="foreignkey")
    op.drop_table("support_problems")
    op.drop_index("ix_support_cases_problema", table_name="support_cases")
    with op.batch_alter_table("support_cases") as batch_op:
        batch_op.drop_constraint("fk_support_case_resp_funcional", type_="foreignkey")
        batch_op.drop_constraint("fk_support_case_resp_tecnico", type_="foreignkey")
        batch_op.drop_constraint("fk_support_case_coordinador", type_="foreignkey")
    for col in (
        "escalamiento_nivel",
        "sla_warning_emitido",
        "servicio_componente",
