...
def test_deterministic_llm_cost_zero(client: TestClient, auth_headers):
    ...
    try:
        ...
        db.add(
            FinOpsRecord(
                organization_id=org_id,
                execution_ref="transversal:deterministic:ops",
...
def test_transversal_deterministic_zero_llm(client: TestClient, auth_headers):
    rows = client.get("/api/finops/planner/transversal", headers=auth_headers).json()
    det_rows = [r for r in rows if r["is_deterministic"]]
    assert det_rows, "Debe existir al menos una capacidad transversal determinística"
...
def test_transversal_estimate_monthly():
    ...
    try:
        ...
        det_items = [i for i in est["items"] if i["is_deterministic"]]
        assert det_items, "Capacidad determinística configurada"
        for item in det_items:
            assert item["cost_ia"] == 0.0
            if item.get("cost_infra"):
                assert item["cost_infra"] > 0
    finally:
        db.close()
